...
class MeetingService:
    ...
    async def store_transcript(
        ...
        """
        try:
            # Get meeting to find channel_id — try Redis first, then MySQL
            meeting = self.get_meeting(meeting_id)
            if not meeting and tenant_name:
                # Meeting may exist only in MySQL (scheduled meeting, Redis expired, etc.)
                try:
                    m_row = await db.execute_query(
                        "SELECT project_id, sprint_id, title, channel_id FROM meetings WHERE meeting_id = %s LIMIT 1",
                        (meeting_id,),
                        schema=tenant_name,
                        fetch_one=True
                    )
                    if m_row:
                        meeting = dict(m_row)
                        logger.info(f"Using MySQL meeting data for transcript storage: {meeting_id}")
                except Exception as _me:
                    logger.warning(f"MySQL meeting lookup for {meeting_id} failed: {_me}")

            if not meeting:
                logger.error(f"Cannot store transcript: Meeting {meeting_id} not found in Redis or MySQL")
                return None
    ...
    async def get_db_channel_transcripts(self, tenant_name: str, channel_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query = """
                ...
                LEFT JOIN meetings m ON t.meeting_id COLLATE utf8mb4_unicode_ci = m.meeting_id COLLATE utf8mb4_unicode_ci
    ...
    async def get_transcript(self, meeting_id: str, tenant_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if tenant_name:
                ...
                if not db_res:
                    # FALLBACK: Try lookup by project_id/sprint_id/title when no direct meeting_id link
                    # Handles transcripts uploaded via /transcripts/upload (meeting_id=NULL)
                    logger.debug(f"No direct meeting_id match for {real_m_id}, trying fallback...")
                    
                    # Get meeting details to find project/sprint
                    ...
                    if m_details:
                        p_id = m_details.get('project_id')
                        s_id = m_details.get('sprint_id')
                        t_title = m_details.get('title')

                        # Level-1: exact match (NULL-safe sprint_id comparison)
                        if s_id is not None:
                            fallback_query = """
                            ...
                            db_res = await db.execute_query(
                                fallback_query, (p_id, s_id, t_title),
                            ...
                            fallback_query = """
                                SELECT DISTINCT t.meeting_id, t.id, t.transcript_content as content, t.title, t.category, 
                                       ...
                                       t.project_id, t.sprint_id
                                FROM transcripts t
                                WHERE t.project_id = %s AND t.sprint_id IS NULL AND t.title = %s
                                ORDER BY t.created_at DESC LIMIT 1
                            """
                            db_res = await db.execute_query(
                                fallback_query, (p_id, t_title),
                                schema=tenant_name, fetch_one=True
                            )

                        # Level-2: project + title only (any sprint — broadest match)
                        if not db_res and p_id:
                            loose_query = """
                                SELECT DISTINCT t.meeting_id, t.id, t.transcript_content as content, t.title, t.category, 
                                       t.created_at as stored_at, t.uploaded_by as created_by,
                                       t.project_id, t.sprint_id
                            ...
                            """
                            db_res = await db.execute_query(
                                loose_query, (p_id, t_title),
                                schema=tenant_name, fetch_one=True
                            )
                            if db_res:
                                logger.info(f"Found transcript via project+title fallback for {real_m_id}")
                ...
                if db_res:
                    ...
                    # Backfill meeting_id on the transcript row so future lookups are fast
                    if not db_res.get('meeting_id') and db_res.get('id'):
                        try:
                            await db.execute_query(
                                "UPDATE `transcripts` SET meeting_id = %s WHERE id = %s AND meeting_id IS NULL",
                                (real_m_id, db_res['id']),
                                schema=tenant_name, commit=True
                            )
                            db_res['meeting_id'] = real_m_id
                        except Exception as _bf:
                            logger.debug(f"Could not backfill meeting_id on transcript: {_bf}")
