        ...
        try:
            ...
            query = """
                INSERT INTO meetings (
                    project_id, sprint_id, title, meeting_category,
            ...
            # Fetch the row we just inserted (UUID generated by DB)
            fetch_query = """
                SELECT *
                FROM meetings
                WHERE project_id = %s AND sprint_id = %s AND title = %s
                  AND meeting_date = %s AND created_by = %s
                ORDER BY created_at DESC
                LIMIT 1
            """
            result = await self.db.execute_query(
                fetch_query,
                (project_id, sprint_id, title, meeting_date, created_by),
                fetch_one=True,
                schema=tenant_name,
            )
